...
contract INTPlaceHolder is TokenController, Owned {
    using SafeMath for uint256;

    MiniMeToken public intoken;
    IntuitionContribution public contribution;
    uint256 public activationTime;
    address public igtExchanger;

    /// @notice Constructor
    /// @param _owner Trusted owner for this contract.
    /// @param _int INT token contract address
    /// @param _contribution IntuitionContribution contract address
    /// @param _igtExchanger IGT-INT Exchange address. (During the first week
    ...
    function changeController(address _newController) public onlyOwner {
        intoken.changeController(_newController);
    ...
    function transferable(address _from) internal returns (bool) {
        ...
        return (getTime() > activationTime) || (_from == igtExchanger);
    }
    ...
    function claimTokens(address _token) public onlyOwner {
        if (intoken.controller() == address(this)) {
            intoken.claimTokens(_token);
        }
        if (_token == 0x0) {
